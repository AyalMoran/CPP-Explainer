 ...
 */
function findKeywordAtPosition(lineText: string, posCh: number): { kw: string; start: number; end: number } | null {
  ...
  while ((m = KEYWORD_REGEX.exec(lineText))) {
    ...
    if (KEYWORD_REGEX.lastIndex === m.index) KEYWORD_REGEX.lastIndex++; // safety
  }
  return null;
}
